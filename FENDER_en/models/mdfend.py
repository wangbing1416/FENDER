...
class MDFENDModel(torch.nn.Module):
    ...
    def forward(self, **kwargs):
        ...
        bert_embedding = self.embedding(input_ids=inputs, token_type_ids=token_type_ids, past_key_values_length=0)
        init_feature = self.bertencoder(bert_embedding)[0]
...
class Trainer():
    ...
    def train(self, logger = None):
        ...
        self.model = MDFENDModel(self.config['emb_dim'], self.config['model']['mlp']['dims'], self.config['model']['mlp']['dropout'])
        ...
        for epoch in range(self.config['epoch']):
            ...
            if mark == 'save':
                torch.save(self.model.state_dict(),
                    os.path.join(self.save_path, 'parameter_mdfend.pkl'))
            elif mark == 'esc':
                break
            else:
                continue
        self.model.load_state_dict(torch.load(os.path.join(self.save_path, 'parameter_mdfend.pkl')))
        ...
        return future_results, os.path.join(self.save_path, 'parameter_mdfend.pkl')
