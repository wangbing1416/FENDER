...
from model import BERTFENDModel, BERT_ENDEFModel, BERTEmoModel, BERTEmo_ENDEFModel, PrefixModel, ODEBlock, ODEFunc, NeuralDynamics
...
class Trainer():
    ...
    def train(self, logger=None):
        ...
        for period in range(1, 10):  # 10 periods by year
            ...
            for epoch in range(self.config['finetuning_epoch']):
                ...
                for step_n, batch in enumerate(train_data_iter):
                    ...
                    if 'endef' in self.config['model_name']:
                        pred, entity_pred, _ = self.model(self.adaptivemodel, **batch_data)
                        loss = loss_fn(pred, label.float()) + self.config['loss_cof'] * loss_fn(entity_pred, label.float())
                    else:
                        pred = self.model(self.adaptivemodel, **batch_data)
                        loss = loss_fn(pred, label.float())
        ...
        self.dynamicsmodel = NeuralDynamics(input_size=self.config['emb_dim'], hidden_size=self.config['dynamics_mlp_dim'])
        if self.config['use_cuda']:
        ...
        for epoch in range(self.config['dynamics_epoch']):
            ...
            pred = self.dynamicsmodel(dynamics_time, x0)
            dynamics_loss = loss_fn_dyn(pred, dynamics_label)
        ...
        test_dataprocess = DataProcess(path=self.config['root_path'] + 'test.json', max_len=self.config['max_len'],
                                       aug_prob=self.config['aug_prob'], para_len=self.config['para_len'], flag='val')
        test_loader =test_dataprocess.get_dataloader(batch_size=self.config['batchsize'], shuffle=False, segment_type=self.config['segment_type'])

        if self.config['para_len'] == 0:
            self.future_prefixmodel = self.prefixmodel
        else:
            future_prefix = self.dynamicsmodel(torch.tensor([future_index]).cuda(), x0).squeeze()
