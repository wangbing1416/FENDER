...
class DataProcess():
    def __init__(self, path, max_len, aug_prob, para_len, flag):
        ...
        self.label = torch.tensor(df_data['label'].apply(lambda c: label_dict[c]).astype(int).to_numpy())
    ...
    def get_dataloader(self, batch_size, shuffle, segment_type, period=0):  # period=0 indicates the overall dataset
        if segment_type == 'year':
            if period > 8:
                raise ValueError('period must not bigger than 10! (2010 - 2017)')
            index = [0, 30, 409, 1595, 2940, 3533, 5299, 6984, 10221]  # start indices in the training set from 2010 to 2017, by year
        else:
            if period > 31:
                raise ValueError('period must not bigger than 37! (2009 - 2018)')
            index = [0, 2, 14, 27, 97, 330, 379, 402, 419, 728, 1124, 1584, 1987, 2371, 2698, 2925, 3187, 3280, 3417, 3514, 3663, 3891, 4104, 5276, 5377, 5790, 6381, 6957, 7806, 8483, 9232, 10190]
        if period == 0:
